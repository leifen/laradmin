    ...
    <div class="row">
        <div class="col-sm-12">
            ...
            <div class="ibox-content">
                ...
                <a href="{{route('users.index')}}"><button class="btn btn-primary btn-sm" type="button"><i class="fa fa-plus-circle"></i> 管理员管理</button></a>
                <div class="hr-line-dashed m-t-sm m-b-sm"></div>
                <form class="form-horizontal m-t-md" action="{{ route('users.store') }}" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
                    {!! csrf_field() !!}
                            ...
                            @foreach($roles as $k=>$item)
                                <label>
                                    <input type="checkbox"
                                           name="role_id[]"
                                           value="{{$item->id}}"
                                           @if($item->id == old('role_id') || in_array($item->id,\Auth::guard('admin')->user()->roles->pluck('id')->toArray()))
                                           checked="checked"
                                            @endif>
                                    {{$item->name}}
                                </label>
